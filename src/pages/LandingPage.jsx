...
export default function LandingPage({ onNav }) {
 ...
 const [stats, setStats] = useState({ doctors: '...', patients: '...', appts: '...', rating: '...' });
 const [showAllDocs, setShowAllDocs] = useState(false);
  ...
  api.getAdminStats().then(data => {
    setStats({
      ...
      appts: data.total_appointments || '0',
  ...
  const renderDocs = searchQ.trim() || showAllDocs ? displayedDocs : displayedDocs.slice(0, 8);
 ...
 {[{ value: stats.doctors, label: 'Verified doctors' }, { value: stats.patients, label: 'Patients' }, { value: stats.appts, label: 'Appointments' }].map(s => (
 <div key={s.label}>
 ...
 {[{ value: stats.doctors, label: 'Verified doctors' }, { value: stats.patients, label: 'Active patients' }, { value: stats.appts, label: 'Appointments booked' }, { value: stats.rating, label: 'Average rating' }].map(s => (
 <div key={s.label}>
 ...
 ) : displayedDocs.length > 8 && (
 <button onClick={() => { setShowAllDocs(!showAllDocs); handleSearch(); }} style={{ background: 'transparent', border: 'none', color: C.primary, fontWeight: 700, fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
 {showAllDocs ? 'Show less' : 'View all doctors'} <ChevronRight size={18} style={{ transform: showAllDocs ? 'rotate(-90deg)' : 'none', transition: 'transform 0.2s' }} />
 </button>
 )}
 </div>
 {renderDocs.length === 0 ? (
 ...
 <div style={{ width: 64, height: 64, borderRadius: '50%', background: `${C.primary}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.primary, flexShrink: 0, overflow: 'hidden' }}>
 {d.avatar ? <img src={d.avatar} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="doctor"/> : <Stethoscope size={28} />}
 </div>
 ...
 <div style={{ fontWeight: 800, color: C.dark, fontSize: 18 }}>EGP {d.consultation_fee || 100}</div>
 ...
 <p style={{ color: '#475569', fontSize: 13 }}>© 2026 MediDash.</p>
 ...
 <div style={{ width: '100%', height: '100%', borderRadius: '50%', background: `${C.primary}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.primary, overflow: 'hidden' }}>
 {docProfile.avatar ? <img src={docProfile.avatar} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="doctor"/> : <Stethoscope size={44} />}
 </div>
 </div>
 ...
 <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: C.gray, fontSize: 15, fontWeight: 500 }}><Activity size={20} /> EGP {docProfile.consultation_fee} / visit</div>
 </div>
