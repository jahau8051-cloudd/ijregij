  ...
  return (
    <div className="fade" style={{ padding: 32 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 16, marginBottom: 28 }}>
        {[
          { icon: <DollarSign size={24} />, label: 'Total Earned', value: `EGP ${parseFloat(earnings.total_earned || 0).toFixed(2)}`, color: C.green },
          { icon: <BarChart2 size={24} />, label: 'This Month', value: `EGP ${parseFloat(earnings.this_month || 0).toFixed(2)}`, color: C.primary },
            ...
            {monthly.map((m, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: C.dark }}>EGP {parseFloat(m.amount).toFixed(0)}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            {appointments.map(a => (
              <div key={a.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 16, padding: '14px 16px', borderBottom: '1px solid #f1f5f9', alignItems: 'center' }}>
                ...
                <div style={{ fontWeight: 800, color: C.green, fontSize: 16 }}>EGP {parseFloat(a.fee || 0).toFixed(2)}</div>
              </div>
            ))}
          </div>
        )}
