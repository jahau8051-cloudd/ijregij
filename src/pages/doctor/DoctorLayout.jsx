...
export default function DoctorLayout({ user, onLogout }) {
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'appointments': return <DoctorAppointments user={user} onPrescribe={handlePrescribe} onMessage={(p) => { setChatTarget(p); setPage('messages'); }} />;
      case 'patients': return <DoctorPatients user={user} onMessage={(p) => { setChatTarget(p); setPage('messages'); }} />;
      case 'prescriptions': return <WritePrescription user={user} initialAppt={prescribeAppt} onClear={() => setPrescribeAppt(null)} />;
      case 'messages': return <DoctorMessages user={user} targetPatient={chatTarget} onClearTarget={() => setChatTarget(null)} />;
      ...
      <aside className="dash-sidebar" style={{ width: 240, background: '#0f172a', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <div className="sidebar-footer" style={{ padding: '16px 12px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', marginBottom: 8 }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#3b82f620', color: '#60a5fa', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
