...
export default function DoctorAppointments({ user, onPrescribe, onMessage }) {
 ...
 const now = new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60000).toISOString().split('T')[0];
 const upcoming = appointments.filter(a => a.status !== 'cancelled' && a.status !== 'completed' && a.appointment_date?.substring(0, 10) >= now).reverse();
 const completed = appointments.filter(a => a.status === 'completed');
 const past = appointments.filter(a => a.status !== 'cancelled' && a.status !== 'completed' && a.appointment_date?.substring(0, 10) < now);
 ...
 return (
 <div key={a.id} style={{ background: '#fff', borderRadius: 14, padding: 'min(5vw, 24px)', boxShadow: '0 1px 8px rgba(0,0,0,0.06)' }}>
 ...
 {a.patient_avatar ? <img src={a.patient_avatar} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="patient" /> : <User size={24} />}
 </div>
 <div style={{ flex: 1 }}>
 ...
 {tab === 'upcoming' && (
 <button onClick={() => onMessage({ id: a.patient_id, name: a.patient_name, avatar: a.patient_avatar })} style={{ padding: '7px 14px', background: `${C.primary}15`, color: C.primary, border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>Message</button>
 )}
 {a.status === 'pending' && (
 <>
