...
import { CheckCircle, UserPlus, User, Stethoscope, FileText, ArrowLeft } from 'lucide-react';
...
export default function SignUpPage({ onSuccess, onNav }) {
 ...
 bio: '', clinicName: '', clinicAddress: '', consultationFee: '', availableDays: 'Sat,Sun,Mon,Tue,Wed', availableFrom: '09:00', availableTo: '17:00'
 });
 ...
 consultationFee: parseInt(form.consultationFee) || 300,
 });
 ...
 <div className="fade auth-box" style={{ position: 'relative', background: '#fff', borderRadius: 20, padding: 48, width: '100%', maxWidth: 480, boxShadow: '0 24px 80px rgba(0,0,0,0.3)', textAlign: 'center' }}>
 <button onClick={() => onNav('landing')} style={{ position: 'absolute', top: 24, left: 24, background: 'rgba(241,245,249,0.7)', border: 'none', borderRadius: 8, padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 6, color: C.gray, fontWeight: 600, fontSize: 13, cursor: 'pointer', transition: 'background .2s' }}>
 ...
 <div className="fade auth-box" style={{ position: 'relative', background: '#fff', borderRadius: 20, padding: 40, width: '100%', maxWidth: 520, boxShadow: '0 24px 80px rgba(0,0,0,0.3)' }}>
 <button onClick={() => { if (step > 1) { setStep(step - 1); setErr(''); } else { onNav('landing'); } }} style={{ position: 'absolute', top: 24, left: 24, background: 'rgba(241,245,249,0.7)', border: 'none', borderRadius: 8, padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 6, color: C.gray, fontWeight: 600, fontSize: 13, cursor: 'pointer', transition: 'background .2s' }}>
   <ArrowLeft size={16} /> Back
 </button>
 ...
 <input style={inp} placeholder="+20 123 456 7890" value={form.phone} onChange={set('phone')} />
 </div>
 ...
 <div style={{ maxHeight: '55vh', overflowY: 'auto', paddingRight: 8, marginBottom: 20 }}>
 <div style={{ background: '#fef9c3', borderRadius: 12, padding: '12px 16px', marginBottom: 24, border: '1px solid #fde68a' }}>
 ...
 <p style={{ fontSize: 11, fontWeight: 700, color: C.gray, textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 12 }}>Professional Info</p>
 ...
 <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 5 }}>Short Bio</label>
 <textarea style={{ ...inp, resize: 'none', minHeight: 60 }} rows="2" placeholder="Tell patients about your expertise..." value={form.bio} onChange={set('bio')} />
 </div>

 <p style={{ fontSize: 11, fontWeight: 700, color: C.gray, textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 12 }}>Clinic Info</p>
 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
 <div style={{ gridColumn: 'span 2' }}>
 <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 5 }}>Clinic Name</label>
 <input style={inp} placeholder="e.g. Cairo Heart Center" value={form.clinicName} onChange={set('clinicName')} />
 </div>
 <div style={{ gridColumn: 'span 2' }}>
 <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 5 }}>Clinic Address</label>
 <input style={inp} placeholder="e.g. 123 Main St, Cairo" value={form.clinicAddress} onChange={set('clinicAddress')} />
 </div>
 <div>
 <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 5 }}>Consultation Fee (EGP)</label>
 <input style={inp} type="number" min="0" placeholder="e.g. 300" value={form.consultationFee} onChange={set('consultationFee')} />
 </div>
 </div>

 <p style={{ fontSize: 11, fontWeight: 700, color: C.gray, textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 12, marginTop: 12 }}>Availability</p>
 <div style={{ marginBottom: 12 }}>
 <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 5 }}>Available Days</label>
 <input style={inp} placeholder="e.g. Sat,Sun,Mon,Tue,Wed" value={form.availableDays} onChange={set('availableDays')} />
 </div>
 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
 <div>
 <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 5 }}>From</label>
 <input style={inp} type="time" value={form.availableFrom} onChange={set('availableFrom')} />
 </div>
 <div>
 <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 5 }}>To</label>
 <input style={inp} type="time" value={form.availableTo} onChange={set('availableTo')} />
 </div>
 </div>

 <p style={{ fontSize: 11, fontWeight: 700, color: C.gray, textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 12 }}>Verification</p>
