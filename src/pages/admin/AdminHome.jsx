...
export default function AdminHome({ onNav }) {
  ...
  const cards = [
    ...
    { icon: <DollarSign size={24} />, label: 'Total Revenue', value: `EGP ${parseFloat(stats.total_revenue || 0).toFixed(2)}`, color: C.dark, sub: `EGP ${parseFloat(stats.today_revenue || 0).toFixed(2)} today` },
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.04)', marginBottom: 24, border: '1px solid #f1f5f9' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 24, fontWeight: 800, color: C.primary }}>EGP {parseFloat(stats.total_revenue || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
            ...
            {monthly.map((m, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: C.dark, marginBottom: 12 }}>EGP {parseFloat(m.revenue).toLocaleString()}</div>
                  <div 
