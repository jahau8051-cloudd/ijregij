...
function AddDoctorPanel({ onClose, onSuccess }) {
 ...
 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
  <div>
  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: C.dark, marginBottom: 4 }}>Consultation fee (EGP)</label>
  <input style={inp} type="number" placeholder="500" value={form.consultationFee} onChange={set('consultationFee')} />
  </div>
 <div>
...
function CredentialModal({ doctor, onClose }) {
 ...
 <div className="fade" style={{ background: '#fff', borderRadius: 16, padding: 32, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,0.3)', position: 'relative' }} onClick={e => e.stopPropagation()}>
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
 <h2 style={{ fontWeight: 800, color: C.dark, fontSize: 18 }}>Doctor Credentials</h2>
 <button onClick={onClose} style={{ position: 'absolute', top: 24, right: 24, width: 32, height: 32, borderRadius: '50%', background: '#f1f5f9', border: 'none', cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>×</button>
 </div>
 <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 24, padding: 16, background: '#f8fafc', borderRadius: 12 }}>
 <div style={{ width: 56, height: 56, borderRadius: 14, background: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, overflow: 'hidden', flexShrink: 0, color: C.primary }}>
   {doctor.avatar ? <img src={doctor.avatar} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="doctor" /> : (
     <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20M17 5H7M19 9H5M21 13H3M19 17H5M17 21H7"/></svg>
   )}
 </div>
 <div>
 ...
 {[
 { label: 'Phone', value: doctor.phone || 'Not provided' },
 { label: 'Clinic Name', value: doctor.clinic_name || 'Not provided' },
 { label: 'Clinic Address', value: doctor.clinic_address || 'Not provided' },
 { label: 'Fee', value: doctor.consultation_fee ? `EGP ${doctor.consultation_fee}` : 'Not provided' },
 { label: 'Available Days', value: doctor.available_days || 'Not provided' },
 { label: 'Hours', value: doctor.available_from && doctor.available_to ? `${doctor.available_from} - ${doctor.available_to}` : 'Not provided' },
 ...
 <span style={{ fontWeight: 600, color: C.dark, fontSize: 14, textAlign: 'right', maxWidth: '60%' }}>{f.value}</span>
 </div>
 ))}
 {doctor.bio && (
 <div style={{ marginTop: 20 }}>
 <p style={{ fontSize: 13, fontWeight: 600, color: C.dark, marginBottom: 8 }}>Biography</p>
 <p style={{ color: C.gray, fontSize: 14, lineHeight: 1.5, background: '#f8fafc', padding: 12, borderRadius: 10 }}>{doctor.bio}</p>
 </div>
 )}
 {doctor.id_document && (
 <div style={{ marginTop: 20 }}>
...
export default function ManageDoctors() {
 ...
 <div style={{ width: 48, height: 48, borderRadius: 14, background: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0, overflow: 'hidden', color: C.primary }}>
   {d.avatar ? <img src={d.avatar} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="doctor"/> : (
     <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20M17 5H7M19 9H5M21 13H3M19 17H5M17 21H7"/></svg>
   )}
 </div>
