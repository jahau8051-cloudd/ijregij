...
function DoctorCard({ doc, onBook }) {
 ...
 <div style={{ width: 68, height: 68, borderRadius: '50%', background: `${C.primary}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, flexShrink: 0, overflow: 'hidden' }}>
 {doc.avatar ? <img src={doc.avatar} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="doctor"/> : (doc.specialty_icon || '')}
 </div>
 <div style={{ flex: 1 }}>
 ...
 {doc.bio && <p dir="auto" style={{ fontFamily: '"Cairo", "Tajawal", system-ui, sans-serif', color: C.dark, fontSize: 14, lineHeight: 1.7, marginBottom: 16, display: '-webkit-box', WebkitLineClamp: 4, WebkitBoxOrient: 'vertical', overflow: 'hidden', opacity: 0.85 }}>{doc.bio}</p>}
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
 <div style={{ fontSize: 22, fontWeight: 800, color: C.dark }}>EGP {doc.consultation_fee}<span style={{ fontSize: 13, fontWeight: 400, color: C.gray }}>/visit</span></div>
 <button onClick={() => onBook(doc)} style={{ background: C.primary, color: '#fff', padding: '10px 24px', borderRadius: 10, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 14, flexShrink: 0 }}>Book Now</button>
 </div>
 </div>
 );
}
...
export default function FindDoctor({ user, onBook }) {
 ...
 <option value="150">Under EGP 150</option>
 <option value="300">Under EGP 300</option>
 <option value="500">Under EGP 500</option>
 <option value="1000">Under EGP 1000</option>
 </select>
