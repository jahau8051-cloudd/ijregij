...
import { Calendar, History, FileText, Heart, Activity, User } from 'lucide-react';
...
export default function PatientHome({ user, onNav, onBook }) {
 ...
 let healthScore = { value: 'Good', color: C.primary, sub: 'General status' };
 if (past.length > 0 && active_rx === 0) {
   healthScore = { value: 'Excellent', color: C.green, sub: 'No active prescriptions' };
 } else if (active_rx > 0) {
   healthScore = { value: 'Monitoring', color: C.orange, sub: `${active_rx} active prescription${active_rx > 1 ? 's' : ''}` };
 } else if (past.length === 0 && upcoming.length > 0) {
   healthScore = { value: 'Pending', color: C.primary, sub: 'Awaiting first checkup' };
 } else if (past.length === 0 && upcoming.length === 0) {
   healthScore = { value: 'Unknown', color: C.gray, sub: 'No medical history yet' };
 }
 ...
 <StatCard icon={<Heart size={24} />} label="Health Score" value={healthScore.value} color={healthScore.color} sub={healthScore.sub} />
 </div>
 ...
 <div style={{ width: 44, height: 44, borderRadius: 12, background: `${C.primary}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', color: C.primary }}>
   {a.doctor_avatar ? <img src={a.doctor_avatar} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="doctor" /> : <User size={20} />}
 </div>
 <div style={{ flex: 1 }}>
