    ...
    <div className="fade" style={{ padding: 32 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 16, marginBottom: 28 }}>
        {[
          ...
          { label: 'Total Spent', value: `EGP ${totalSpent.toFixed(2)}`, icon: <DollarSign size={24} />, color: C.green },
          { label: 'This Month', value: `EGP ${payments.filter(p => new Date(p.created_at).getMonth() === new Date().getMonth()).reduce((s, p) => s + parseFloat(p.amount || 0), 0).toFixed(2)}`, icon: <Calendar size={24} />, color: '#f59e0b' },
        ].map(s => (
        ...
        <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
          ...
          {payments.map(p => (
            <div key={p.id} style={{ padding: '18px 24px', borderBottom: '1px solid #f1f5f9', display: 'grid', gridTemplateColumns: '1fr 1fr auto auto auto', gap: 16, alignItems: 'center' }}>
              ...
              <div style={{ fontWeight: 800, color: C.green, fontSize: 16 }}>EGP {parseFloat(p.amount).toFixed(2)}</div>
            </div>
          ))}
        </div>
      )}
